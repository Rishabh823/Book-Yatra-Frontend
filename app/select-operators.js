...
const OP_AVATAR_COLORS = ['#3730A3', '#0F766E', '#7C3AED', '#B45309', '#0369A1', '#15803D'];
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <View style={s.topHeader}>
        ...
        <Text style={s.topSub}>Select the tour operators you want to follow</Text>
      </View>
        ...
        <View style={{ flexDirection: 'row', gap: 12 }}>
          ...
          {selected.size > 0 && (
            <TouchableOpacity onPress={clearAll}>
              <Text style={[s.controlLink, { color: '#9CA3AF' }]}>Clear</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
      ...
      {loading ? (
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 32 }}>
          ...
          <Text style={{ fontFamily: fonts.bodyBold, fontSize: 16, color: '#374151', marginTop: 16, textAlign: 'center' }}>
            No operators available yet
          </Text>
          <Text style={{ fontFamily: fonts.body, fontSize: 13, color: '#9CA3AF', marginTop: 6, textAlign: 'center' }}>
      ...
      {/* Bottom actions */}
      <View style={s.footer}>
        <TouchableOpacity style={s.cancelBtn} onPress={onSkip} testID="skip-btn">
          <Text style={s.cancelTxt}>{isFirstTime ? 'Skip for now' : 'Cancel'}</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={[s.saveBtn, (!hasChanges() && !isFirstTime) && s.saveBtnDisabled]}
          onPress={onSave}
        ...
        >
          {saving ? <ActivityIndicator color="#fff" /> : (
            <>
              <Text style={s.saveBtnTxt}>{isFirstTime ? 'Continue' : 'Save changes'}</Text>
