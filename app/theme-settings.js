            ...
            <View style={styles.accentGrid}>
              {ACCENT_THEMES.map(accentTheme => {
                ...
                return (
                  <TouchableOpacity
                    key={accentTheme.id}
                    style={[styles.accentCard, isActive && styles.accentCardActive]}
                    onPress={() => handleSetAccent(accentTheme.id)}
                    activeOpacity={0.75}
                  >
                    <View style={[styles.accentSwatch, { backgroundColor: accentTheme.primary }]}>
                      {isActive && <Ionicons name="checkmark" size={16} color="white" />}
                    </View>
                    <Text style={styles.accentEmoji}>{accentTheme.emoji}</Text>
                    <Text style={[styles.accentName, isActive && { color: accentTheme.primary }]}>
                      {accentTheme.name}
                    </Text>
                  </TouchableOpacity>
                );
