    ...
    >
      <View style={styles.header}>
        ...
        <View style={[styles.typeBadge, { backgroundColor: typeStyle.bg }]}>
          <Text style={[styles.typeText, { color: typeStyle.color }]}>{(post.type || '').replace('_', ' ')}</Text>
        </View>
      </View>
      {post.title ? <Text style={styles.title} numberOfLines={2}>{post.title}</Text> : null}
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.action} onPress={() => onLike?.(post._id)}>
          <Ionicons name={post.isLiked ? 'heart' : 'heart-outline'} size={16} color={post.isLiked ? '#EF4444' : '#6B7280'} />
          <Text style={[styles.actionCount, post.isLiked && { color: '#EF4444' }]}>{post.likes || 0}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#D95D39', alignItems: 'center', justifyContent: 'center' },
  ...
  typeText: { fontFamily: 'Manrope_700Bold', fontSize: 10, textTransform: 'capitalize' },
  title: { fontFamily: 'Philosopher_700Bold', fontSize: 16, color: '#1A1A1A' },
  content: { fontFamily: 'Manrope_400Regular', fontSize: 13, color: '#6B7280', lineHeight: 19 },
  tagsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 4 },
  tag: { backgroundColor: '#FEE8E2', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 999 },
  tagText: { fontFamily: 'Manrope_500Medium', fontSize: 11, color: '#D95D39' },
  footer: { flexDirection: 'row', gap: 16, borderTopWidth: 1, borderTopColor: '#F3F4F6', paddingTop: 8 },
  action: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  actionCount: { fontFamily: 'Manrope_500Medium', fontSize: 13, color: '#6B7280' },
});
