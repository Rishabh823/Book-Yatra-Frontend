...
export const useSocket = (namespace = "") => {
  ...
  const connect = useCallback(async () => {
    // Reuse existing connected socket rather than creating a new one
    ...
    // Wait for connection so callers can immediately emit events
    await new Promise((resolve) => {
      ...
      socketRef.current.once("connect_error", resolve); // don't hang on error
    });
