// Centralized API client for Book Yatra backend
...
export const auth = {
  ...
  login: async (emailOrMobile, password) => {
    const res = await api.post("/users/login-user", { emailOrMobile, password });
    if (res?.token) {
  ...
  verifyRegOtp: (email, otp) => api.post("/users/verify-reg-otp", { email, otp }),
  ...
  joinOperators: (operatorIds) => api.post("/users/join-operators", { operatorIds }),
...
export const bookings = {
  ...
  requestRefund: (bookingId, text) => api.post(`/bookings/${bookingId}/refund-request`, { text }),
};
...
export const preferencesApi = {
  ...
  getFavorites:    (page = 1) => api.get(`/preferences/favorites?page=${page}&limit=20`),
  ...
  addSearchHistory:   (query) => api.post('/preferences/search-history', { query }),
  clearSearchHistory: ()      => api.del('/preferences/search-history'),
  getSuggestions:     (q)     => api.get(`/preferences/search-suggestions?q=${encodeURIComponent(q)}`),
};
...
export const securityApi = {
  ...
  verifyAndEnableMFA:(token)  => api.post('/security/mfa/verify-setup', { token }),
  ...
  getActivity:       (page=1) => api.get(`/security/activity?page=${page}&limit=30`),
};
...
export const vehiclesApi = {
  ...
  assignDriver:   (id, dId) => api.put('/vehicles/' + id + '/assign-driver', { driverId: dId }),
};
...
export const trackingApi = {
  start:          (tourId, data)       => api.post('/tracking/start/' + tourId, data),
  stop:           (tourId)             => api.put('/tracking/stop/' + tourId, {}),
  updateLocation: (tourId, loc)        => api.put('/tracking/location/' + tourId, loc),
  getForTour:     (tourId)             => api.get('/tracking/' + tourId),
...
export const chatApi = {
  ...
  messages:   (chatId, page=1)  => api.get('/chat/' + chatId + '/messages?page=' + page),
  send:       (data)            => api.post('/chat/message', data),
};

export const communityApi = {
  feed:       (type, page=1)  => api.get('/community?type=' + type + '&page=' + page),
...
export const volunteerApi = {
  ...
  passengers:    (tourId)         => api.get('/volunteer/passengers?tourId=' + tourId),
