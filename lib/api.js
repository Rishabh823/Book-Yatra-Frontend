...
export const superAdmin = {
  ...
  notificationHistory: (page = 1) => api.get(`/notifications/broadcast/history?page=${page}`),
  // Refund management
  allRefunds: (status = "") => api.get(`/bookings?refundRequested=true${status ? `&refundStatus=${status}` : ""}`),
  approveRefund: (id, data) => api.post(`/bookings/${id}/refund/approve`, data),
  rejectRefund: (id, reason) => api.post(`/bookings/${id}/refund/reject`, { reason }),
  // Operator commission
  setCommission: (operatorId, rate) => api.put(`/users/${operatorId}/commission`, { commissionRate: rate }),
};
...
export const trackingApi = {
  ...
  updateLocation: (tourId, loc) => api.post("/tracking/location", { tourId, ...(loc || {}) }),
...
export const walletApi = {
  ...
  transactions: (page = 1, params = "") => api.get(`/wallet/transactions?page=${page}&limit=20&${params}`),
...
export const operatorWalletApi = {
  ...
  analytics: (months = 6) => api.get(`/operator-wallet/analytics?months=${months}`),
  settlements: (page = 1) => api.get(`/operator-wallet/settlements?page=${page}`),
  ...
  withdrawals: (page = 1) => api.get(`/operator-wallet/withdrawals?page=${page}`),
};

// ─── Super Admin Finance / Settlements ────────────────────────────────────────
export const settlementApi = {
  dashboard: () => api.get("/settlements/finance/dashboard"),
  withdrawals: (status = "", page = 1) => api.get(`/settlements/withdrawals?status=${status}&page=${page}`),
  approveWithdrawal: (id, data) => api.post(`/settlements/withdrawals/${id}/approve`, data),
  rejectWithdrawal: (id, reason) => api.post(`/settlements/withdrawals/${id}/reject`, { reason }),
  operatorEarnings: (id) => api.get(`/settlements/operator/${id}/earnings`),
  history: (page = 1, operatorId = "") => api.get(`/settlements/history?page=${page}&operatorId=${operatorId}`),
};
