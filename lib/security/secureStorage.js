...
export const pinStorage = {
  ...
  async getBiometricEnabled() {
    // If we have a user-scoped key, check it first.
    if (this._userId) {
      const flag = await secureStorage.get(this.BIO_KEY);
      if (flag !== null) return flag?.enabled === true;

      // Not found under user key — check the generic key written during onboarding
      // when userId was not yet available, and migrate it on the spot.
      const generic = await secureStorage.get("biometric_enabled");
      if (generic !== null) {
        await secureStorage.set(this.BIO_KEY, generic);
        await secureStorage.remove("biometric_enabled");
        return generic?.enabled === true;
      }
      return false;
    }
    // No userId set yet — fall back to generic key.
    const flag = await secureStorage.get(this.BIO_KEY); // resolves to "biometric_enabled"
