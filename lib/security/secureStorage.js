...
export const secureStorage = {
  ...
  async clearAll(keys) {
    await Promise.allSettled(keys.map(k => SecureStore.deleteItemAsync(PREFIX + k)));
  },
};

// ─── PIN helpers (hash lives only on device) ─────────────────────────────────

import * as Crypto from 'expo-crypto';

export const pinStorage = {
  KEY: 'pin_hash',
  PIN_LENGTH_KEY: 'pin_length',
  ATTEMPTS_KEY: 'pin_failed_attempts',
  LOCKED_KEY: 'pin_locked_until',
