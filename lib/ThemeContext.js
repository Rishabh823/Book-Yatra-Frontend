...
const RESOLVED_KEY = "theme_resolved"; // last known resolved mode — used as fallback on cold start

export const ThemeProvider = ({ children }) => {
  const [modePreference, setModePreference] = useState("system"); // user choice
  const [accentId, setAccentId] = useState("saffron");
  // lastResolved: the resolved mode from the previous session, loaded from AsyncStorage.
  // On Android, useColorScheme() and Appearance.getColorScheme() can both return null
  // on cold starts after the first install, causing the theme to incorrectly fall back to
  // "light". Using the previously persisted resolved mode prevents this flash.
  const [lastResolved, setLastResolved] = useState(null);

  const rawDeviceScheme = useColorScheme();
  const deviceColorScheme =
    rawDeviceScheme ?? Appearance.getColorScheme() ?? lastResolved ?? "light";

  // Load saved preferences
  useEffect(() => {
    (async () => {
      const [savedMode, savedAccent, savedResolved] = await Promise.all([
        AsyncStorage.getItem(PREF_KEY),
        AsyncStorage.getItem(ACCENT_KEY),
        AsyncStorage.getItem(RESOLVED_KEY),
      ]);
      if (savedResolved) setLastResolved(savedResolved);
  ...
  const resolvedMode =
    modePreference === "system" ? (deviceColorScheme || "light") : modePreference;
  const theme = buildTheme(resolvedMode, accentId);

  // Persist the resolved mode so it can be used as fallback on next cold start
  useEffect(() => {
    AsyncStorage.setItem(RESOLVED_KEY, resolvedMode);
  }, [resolvedMode]);
