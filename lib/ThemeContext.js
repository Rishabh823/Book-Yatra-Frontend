...
export const ThemeProvider = ({ children }) => {
  ...
  // Initialised synchronously from Appearance.getColorScheme() so the very
  // first render always gets the correct OS theme — no null, no flash.
  // Appearance.addChangeListener then keeps this in sync whenever the OS
  // theme changes (works without an app restart, like WhatsApp/Instagram).
  const [deviceScheme, setDeviceScheme] = useState(
    () => Appearance.getColorScheme() ?? "light",
  );

  // Keep deviceScheme live — fires immediately on OS theme change.
  useEffect(() => {
  ...
  // Load the user's saved preference (light / dark / system) on mount.
  useEffect(() => {
    (async () => {
  ...
  //   "system" → follow OS    "light" / "dark" → user override
  const resolvedMode =
    modePreference === "system" ? deviceScheme : modePreference;
